...
def load_data_and_model():
    ...
    item_properties = pd.concat([pd.read_csv('item_properties_part1.csv'),
                                 pd.read_csv('item_properties_part2.csv')])
    # Filter for categoryid only
    item_properties = item_properties[item_properties['property'] == 'categoryid'][
        ['itemid', 'value']].drop_duplicates()
    model = tf.keras.models.load_model('rnn_recommender_optimized.h5')
    with open('item_tokenizer_final.pkl', 'rb') as f:
        item_tokenizer = pickle.load(f)
    with open('category_tokenizer_final.pkl', 'rb') as f:
        category_tokenizer = pickle.load(f)
    return events_df, item_properties, model, item_tokenizer, category_tokenizer


events_df, item_properties, model, item_tokenizer, category_tokenizer = load_data_and_model()

# Filter events to include only items with valid categoryid
valid_items = item_properties['itemid'].astype(str).unique()
events_df = events_df[events_df['event'].isin(['addtocart', 'transaction'])]
events_df = events_df[events_df['itemid'].astype(str).isin(valid_items)]
events_df = events_df.sort_values(by=['visitorid', 'timestamp'])

# Merge categories with events
item_categories = item_properties[['itemid', 'value']].drop_duplicates()
item_categories['value'] = item_categories['value'].astype(str)
events_df = events_df.merge(item_categories, on='itemid', how='inner')
...
user_sequences = user_sequences[(user_sequences['sequence_length'] >= 3) & (user_sequences['sequence_length'] <= 28)]
sampled_visitors = user_sequences.sample(n=100, random_state=42)['visitorid'].tolist()
...
if input_type == "Select Visitor ID":
    selected_visitor = st.selectbox("Select a Visitor ID:", sampled_visitors)
    if selected_visitor:
        user_data = events_df[events_df['visitorid'] == selected_visitor][['itemid', 'value', 'timestamp']].sort_values(
            'timestamp')
    ...
    if search_term:
        filtered_visitors = [vid for vid in user_sequences['visitorid'] if str(vid).startswith(search_term)]
        if filtered_visitors:
            selected_visitor = st.selectbox("Select a matching Visitor ID:", filtered_visitors)
            user_data = events_df[events_df['visitorid'] == selected_visitor][
                ['itemid', 'value', 'timestamp']].sort_values('timestamp')
            item_sequence = user_data['itemid'].astype(str).tolist()
            category_sequence = user_data['value'].astype(str).tolist()
            st.write(f"**Your Sequence**: {', '.join(item_sequence)}")
            st.write(f"**Categories**: {', '.join(category_sequence)}")
    ...
    if item_sequence:
        for item in item_sequence:
            if item in item_properties['itemid'].astype(str).values:
                cat = item_properties[item_properties['itemid'] == int(item)]['value'].iloc[0]
                category_sequence.append(str(cat))
            else:
                category_sequence.append('0')  # Fallback for manual input
        st.write(f"**Your Sequence**: {', '.join(item_sequence)}")
        st.write(f"**Categories**: {', '.join(category_sequence)}")
...
def get_recommendations(item_seq, cat_seq, max_sequence_length=28):
    ...
    recommended_items = [reverse_item_map.get(idx, 'Unknown') for idx in top_5_indices]

    # Get categories
    categories = []
    for item in recommended_items:
        if item != 'Unknown' and item in item_properties['itemid'].astype(str).values:
            cat = item_properties[item_properties['itemid'] == int(item)]['value'].iloc[0]
        else:
            cat = 'No Category'
    ...
    item_names = [f"Item {item}" if item != 'Unknown' else 'Unknown Product' for item in recommended_items]
...
if st.button("Get Recommendations 🎉"):
    ...
    if error:
        ...
        # Visualize confidence scores (vertical bar plot with Matplotlib, bars from y=0 upward)
        ...
        ax.set_ylim(0, 1)  # Ensure y-axis starts at 0 and goes up to 1
